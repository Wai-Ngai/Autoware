  ...
  const double yaw_threshold = std::numeric_limits<double>::max())
{
  ...
  {  // with dist and yaw thresholds
    ...
    // 遍历所有轨迹点，计算自车到该点的欧氏距离，自车与该点航向角之差，找到距离最小且满足阈值的点
    for (size_t i = 0; i < points.size(); ++i) {
      const auto squared_dist = tier4_autoware_utils::calcSquaredDistance2d(points.at(i), pose.position);
      const auto yaw = tier4_autoware_utils::calcYawDeviation(tier4_autoware_utils::getPose(points.at(i)), pose);

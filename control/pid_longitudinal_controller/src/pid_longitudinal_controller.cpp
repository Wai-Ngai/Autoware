...
{
  using std::placeholders::_1;  // 第一个参数的占位符
  ...
  m_enable_large_tracking_error_emergency = node.declare_parameter<bool>("enable_large_tracking_error_emergency");
  m_enable_slope_compensation = node.declare_parameter<bool>("enable_slope_compensation");
  m_enable_keep_stopped_until_steer_convergence = node.declare_parameter<bool>("enable_keep_stopped_until_steer_convergence");
  ...
  // parameters for state transition
  {
    ...
    p.drive_state_offset_stop_dist = node.declare_parameter<double>("drive_state_offset_stop_dist");                // [m]
    // stopping
    p.stopping_state_stop_dist = node.declare_parameter<double>("stopping_state_stop_dist");                        // [m]
    p.stopped_state_entry_duration_time = node.declare_parameter<double>("stopped_state_entry_duration_time");      // [s]
    // stop
    p.stopped_state_entry_vel = node.declare_parameter<double>("stopped_state_entry_vel");                          // [m/s]
    p.stopped_state_entry_acc = node.declare_parameter<double>("stopped_state_entry_acc");                          // [m/s²]
    // emergency
    p.emergency_state_overshoot_stop_dist = node.declare_parameter<double>("emergency_state_overshoot_stop_dist");  // [m]
    p.emergency_state_traj_trans_dev = node.declare_parameter<double>("emergency_state_traj_trans_dev");            // [m]
    p.emergency_state_traj_rot_dev = node.declare_parameter<double>("emergency_state_traj_rot_dev");                // [m]
  }

  // parameters for drive state
  {
    // 这里使用了c++11的 列表初始化 进行赋值，要求右侧表达式的类型能够直接转换为左侧变量的类型，且不允许发生信息损失。
    // initialize PID gain
    ...
    m_enable_integration_at_low_speed = node.declare_parameter<bool>("enable_integration_at_low_speed");
    m_current_vel_threshold_pid_integrate = node.declare_parameter<double>("current_vel_threshold_pid_integration");  // [m/s]

    m_time_threshold_before_pid_integrate = node.declare_parameter<double>("time_threshold_before_pid_integration");  // [s]

    m_enable_brake_keeping_before_stop = node.declare_parameter<bool>("enable_brake_keeping_before_stop");            // [-]
    m_brake_keeping_acc = node.declare_parameter<double>("brake_keeping_acc");                                        // [m/s^2]
  }

  // parameters for smooth stop state
  {
    const double max_strong_acc{node.declare_parameter<double>("smooth_stop_max_strong_acc")};      // [m/s^2]
    const double min_strong_acc{node.declare_parameter<double>("smooth_stop_min_strong_acc")};      // [m/s^2]
    const double weak_acc{node.declare_parameter<double>("smooth_stop_weak_acc")};                  // [m/s^2]
    const double weak_stop_acc{node.declare_parameter<double>("smooth_stop_weak_stop_acc")};        // [m/s^2]
    const double strong_stop_acc{node.declare_parameter<double>("smooth_stop_strong_stop_acc")};    // [m/s^2]

    const double max_fast_vel{node.declare_parameter<double>("smooth_stop_max_fast_vel")};          // [m/s]
    const double min_running_vel{node.declare_parameter<double>("smooth_stop_min_running_vel")};    // [m/s]
    const double min_running_acc{node.declare_parameter<double>("smooth_stop_min_running_acc")};    // [m/s^2]
    const double weak_stop_time{node.declare_parameter<double>("smooth_stop_weak_stop_time")};      // [s]

    const double weak_stop_dist{node.declare_parameter<double>("smooth_stop_weak_stop_dist")};      // [m]
    const double strong_stop_dist{node.declare_parameter<double>("smooth_stop_strong_stop_dist")};  // [m]
  ...
  m_max_acc = node.declare_parameter<double>("max_acc");        // [m/s^2]  3.0
  m_min_acc = node.declare_parameter<double>("min_acc");        // [m/s^2] -5.0
  ...
  m_ego_nearest_dist_threshold = node.has_parameter("ego_nearest_dist_threshold")
                               ? node.get_parameter("ego_nearest_dist_threshold").as_double()
                               : node.declare_parameter<double>("ego_nearest_dist_threshold");  // [m]
  m_ego_nearest_yaw_threshold = node.has_parameter("ego_nearest_yaw_threshold")
...
void PidLongitudinalController::setCurrentAcceleration(const geometry_msgs::msg::AccelWithCovarianceStamped & msg)
{
  m_current_accel = msg;
}
...
void PidLongitudinalController::setTrajectory(const autoware_auto_planning_msgs::msg::Trajectory & msg)
{
  // 所有轨迹信息均是有限，且非空的，则轨迹有效
...
// [[maybe_unused]]修饰符，表示该参数在函数体中未被使用。这个修饰符用于告诉编译器忽略未使用的参数，并避免产生编译警告。
...
{
  // set input data
  setTrajectory(input_data.current_trajectory);               // 判断轨迹是否有效，并赋值给成员变量
  ...
  // self pose is far from trajectory 计算紧急停车命令
  ...
  const geometry_msgs::msg::Pose & current_pose)
{
  ...
  const auto & nearest_point = m_trajectory.points.at(nearest_idx).pose; // 返回的实际上是一个Pose类型的对象,使用引用，可以直接将nearest_point指向该对象的内存地址，而不进行任何拷贝构造。

  // check if the deviation is worth emergency 检查偏差是否值得紧急处理
  m_diagnostic_data.trans_deviation = tier4_autoware_utils::calcDistance2d(nearest_point, current_pose);
  const bool is_dist_deviation_large = m_state_transition_params.emergency_state_traj_trans_dev < m_diagnostic_data.trans_deviation;
  ...
  const bool is_yaw_deviation_large = m_state_transition_params.emergency_state_traj_rot_dev < m_diagnostic_data.rot_deviation;
  ...
  const double traj_pitch = longitudinal_utils::getPitchByTraj(m_trajectory, control_data.nearest_idx, m_wheel_base);
...
// 紧急状态停车命令计算
PidLongitudinalController::Motion PidLongitudinalController::calcEmergencyCtrlCmd(const double dt) const
{
  // These accelerations are without slope compensation
  const auto & p = m_emergency_state_params;

  const double vel = longitudinal_utils::applyDiffLimitFilter(p.vel, m_prev_raw_ctrl_cmd.vel, dt, p.acc);
  const double acc = longitudinal_utils::applyDiffLimitFilter(p.acc, m_prev_raw_ctrl_cmd.acc, dt, p.jerk);
...
void PidLongitudinalController::updateControlState(const ControlData & control_data)
{
  ...
  const bool departure_condition_from_stopping = stop_dist > p.drive_state_stop_dist + p.drive_state_offset_stop_dist;
  ...
  if (m_control_state == ControlState::DRIVE) {
    ...
    if (m_enable_smooth_stop) {
      if (stopping_condition) {
        ...
        const double pred_vel_in_target = predictedVelocityInTargetPoint(control_data.current_motion, m_delay_compensation_time);
        const double pred_stop_dist = control_data.stop_dist - 0.5 * (pred_vel_in_target + current_vel) * m_delay_compensation_time;
  ...
  const geometry_msgs::msg::Pose & current_pose, const ControlData & control_data)
{
  ...
  if (m_control_state == ControlState::DRIVE) {
    ...
    target_motion = Motion{target_interpolated_point.longitudinal_velocity_mps,
                           target_interpolated_point.acceleration_mps2};
    // 有停止点，从当前点到停止点，加速度保持-0.2或轨迹最小加速度
    target_motion = keepBrakeBeforeStop(m_trajectory, target_motion, nearest_idx);

    // 计算延迟补偿时间0.17s后，自车的速度
    const double pred_vel_in_target = predictedVelocityInTargetPoint(control_data.current_motion, m_delay_compensation_time);
    m_debug_values.setValues(DebugValues::TYPE::PREDICTED_VEL, pred_vel_in_target);

    // 反馈 + 前馈 计算目标加速度
    raw_ctrl_cmd.vel = target_motion.vel;
    raw_ctrl_cmd.acc = applyVelocityFeedback(target_motion, control_data.dt, pred_vel_in_target, control_data.shift);
    
    RCLCPP_DEBUG(
      logger_,
      "[feedback control]  vel: %3.3f, acc: %3.3f, dt: %3.3f, v_curr: %3.3f, v_ref: %3.3f "
      "feedback_ctrl_cmd.ac: %3.3f",
      raw_ctrl_cmd.vel, raw_ctrl_cmd.acc, control_data.dt, current_vel, target_motion.vel,raw_ctrl_cmd.acc);
    ...
    raw_ctrl_cmd.acc = m_smooth_stop.calculate(control_data.stop_dist, current_vel, current_acc, m_vel_hist, m_delay_compensation_time);
    ...
    // stopped 状态 直接请求停车状态的加速度 stopped_vel: 0.0  stopped_acc: -3.4 很小，保证斜坡可以停住
    // This acceleration is without slope compensation
    const auto & p = m_stopped_state_params;
    raw_ctrl_cmd.vel = p.vel;
    raw_ctrl_cmd.acc = longitudinal_utils::applyDiffLimitFilter(p.acc, m_prev_raw_ctrl_cmd.acc, 
  ...
  const Motion & ctrl_cmd, const double & current_vel)
{
  ...
  cmd.speed = static_cast<decltype(cmd.speed)>(ctrl_cmd.vel);               // decltype(cmd.speed)来获取cmd.speed的数据类型
...
double PidLongitudinalController::getDt()
{
  ...
  if (!m_prev_control_time) {
    ...
    // 非首次运行，计算上周期控制到当前的时间差
    dt = (clock_->now() - *m_prev_control_time).seconds();
    *m_prev_control_time = clock_->now();
  }
  const double max_dt = m_longitudinal_ctrl_period * 2.0;   // 20ms
  const double min_dt = m_longitudinal_ctrl_period * 0.5;   // 5ms
  ...
  const double raw_acc, const ControlData & control_data)
{
  ...
  // 加速度坡度补偿
  const double acc_slope_filtered = applySlopeCompensation(acc_max_filtered, control_data.slope_angle, control_data.shift);
  ...
  const size_t nearest_idx) const
{
  ...
  // 找到轨迹中的停车点（目标速度为0）index
  const auto stop_idx = motion_utils::searchZeroVelocityIndex(traj.points);
  if (!stop_idx) {
    return output_motion; // 没有停车点，直接返回
  }

  // 遍历规划轨迹，找到从停车点到起始点的最小加速度
  ...
  const double brake_keeping_acc = std::max(m_brake_keeping_acc, min_acc_before_stop); // -0.2
  ...
  const Motion current_motion, const double delay_compensation_time) const
{
  ...
  const auto past_delay_time = clock_->now() - rclcpp::Duration::from_seconds(delay_compensation_time);
  ...
  const Motion target_motion, const double dt, const double current_vel, const Shift & shift)
{
  ...
  const bool enable_integration = (vehicle_is_moving || (m_enable_integration_at_low_speed && vehicle_is_stuck)) &&
                                  is_under_control;

  // 速度误差滤波                               
  const double error_vel_filtered = m_lpf_vel_error->filter(diff_vel);

  std::vector<double> pid_contributions(3);  // 初始容量是3
  
  // 调用PID控制器计算
  const double pid_acc = m_pid_vel.calculate(error_vel_filtered, dt, enable_integration, pid_contributions);
  ...
  const double ff_scale = std::clamp(std::abs(current_vel) / std::max(std::abs(target_motion.vel), 0.1), 

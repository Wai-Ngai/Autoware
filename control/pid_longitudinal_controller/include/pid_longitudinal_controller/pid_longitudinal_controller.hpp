...
// 使用了嵌套命名空间（nested namespace）的语法，将autoware::motion::control::trajectory_follower命名空间重命名为trajectory_follower
...
private:
   ...
   * @param [in] control_data control data
   */
  Motion calcCtrlCmd(const geometry_msgs::msg::Pose & current_pose, 
   ...
   */
  double predictedVelocityInTargetPoint(const Motion current_motion, 
  ...
  double applyVelocityFeedback(const Motion target_motion, const double dt, 
                               const double current_vel, const Shift & shift);

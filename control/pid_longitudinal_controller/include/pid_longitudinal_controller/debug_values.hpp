...
public:
  ...
  // 枚举类型TYPE，并使用了C++11引入的enum class语法来定义
  enum class TYPE {
    DT = 0,
    CURRENT_VEL = 1,
    TARGET_VEL = 2,
   ...
   */
  std::array<double, static_cast<size_t>(TYPE::SIZE)> getValues() const { 
    return m_values; 
  }
   ...
   * @param [in] value value to set
   */
  void setValues(const size_t type, const double value) { 
    m_values.at(type) = value; 
  }

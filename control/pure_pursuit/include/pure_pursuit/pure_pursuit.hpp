...
private:
  // variables for debug
  geometry_msgs::msg::Point loc_next_wp_;    // 预瞄点位姿
  geometry_msgs::msg::Point loc_next_tgt_;   // 预瞄点位置

  // variables got from outside
  double lookahead_distance_, closest_thr_dist_, closest_thr_ang_;        // 预瞄距离、最近点距离误差阈值、最近点航向角误差阈值
  std::shared_ptr<std::vector<geometry_msgs::msg::Pose>> curr_wps_ptr_;   // 规划的轨迹点
  std::shared_ptr<geometry_msgs::msg::Pose> curr_pose_ptr_;               // 自车当前位置

  // functions
  int32_t findNextPointIdx(int32_t search_start_idx);                              // 最近点的index
  std::pair<bool, geometry_msgs::msg::Point> lerpNextTarget(int32_t next_wp_idx);  // 线性插值计算下一个跟踪点
};

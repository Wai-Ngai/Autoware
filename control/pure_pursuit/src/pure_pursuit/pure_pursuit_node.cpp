...
: Node("pure_pursuit", node_options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
{
  ...
  param_.reverse_min_lookahead_distance = this->declare_parameter<double>("reverse_min_lookahead_distance");
...
boost::optional<double> PurePursuitNode::calcTargetCurvature()
{
  ...
  const double min_lookahead_distance = is_reverse ? param_.reverse_min_lookahead_distance 
                                                   : param_.min_lookahead_distance;
  const double lookahead_distance = calcLookaheadDistance(current_odometry_->twist.twist.linear.x,
                                                          param_.lookahead_distance_ratio,
                                                          min_lookahead_distance);

  // Set PurePursuit data
  pure_pursuit_->setCurrentPose(current_pose_->pose);                         // 自车位置
  pure_pursuit_->setWaypoints(planning_utils::extractPoses(*trajectory_));    // 规划轨迹
  pure_pursuit_->setLookaheadDistance(lookahead_distance);                    // 预瞄距离
...
boost::optional<autoware_auto_planning_msgs::msg::TrajectoryPoint> PurePursuitNode::calcTargetPoint() const
{
